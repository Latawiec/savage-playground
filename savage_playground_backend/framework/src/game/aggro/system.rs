...
use super::{component::AggroTable, event::AggroChangeEvent};
...
pub fn aggro_system(
    ...
    const TAUNT_FIRST_TARGET_DEFAULT_VALUE: u32 = 100;
        ...
        match ev {
            ...
            AggroChangeEvent::Taunt { player, target } => {
                if let Ok(mut target_aggro_table) = query_aggro_table.get_mut(*target) {
                    ...
                    match current_top {
                        Some(top) => {
                            let current_aggro = target_aggro_table
                                .get_aggro_for_entity(*target)
                                .unwrap_or(0);
                            let increase = (top.aggro as f32 * TAUNT_OVERTAKE_MULTIPLIER) as u32
                                - current_aggro;
                            target_aggro_table.increase_aggro(player, increase);
                        }
                        None => target_aggro_table
                            .increase_aggro(player, TAUNT_FIRST_TARGET_DEFAULT_VALUE),
                    };
                } else {
