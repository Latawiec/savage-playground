...
use super::{
    component::drawable::{Drawable, Sprite},
...
pub fn render_snapshot_system(
    ...
    for (entity, transform, drawable, sprite_opt) in drawable.iter() {
        ...
        map.insert(
            "drawable".to_owned(),
            serde_json::to_value(drawable).unwrap(),
        );
    ...
    ev_renderer_snapshot.send(RendererSnapshot {
        snapshot: serde_json::Value::Object(snapshot_map),
    })
}
