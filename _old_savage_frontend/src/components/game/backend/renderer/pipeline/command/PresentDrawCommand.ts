...
import screenSpaceRectSrc from '../../../assets/local_assets/mesh/screen_space_rect.json'
import blitTextureVsSrc from '../../../assets/local_assets/shader/blit_texture.vs.glsl'
import blitTexturePsSrc from '../../../assets/local_assets/shader/blit_texture.ps.glsl'
...
export class PresentDrawCommand implements IDrawCommand {
    ...
    static async fromResources (resources: CommitedResourceStorage, texture: WebGLTexture): Promise<PresentDrawCommand> {
      try {
        const program = await resources.programs.read(blitTextureVsSrc, blitTexturePsSrc)
        const mesh = await resources.meshes.read(screenSpaceRectSrc)
